...
public class TownSquare extends Room {
    ...
    public String enter() {
    	System.out.println("The Town Square is bustling with people. A young man in rags shouts the latest news."
    		...
    		+ "To the (e)ast is a squat building whose sign reads 'Apothecary.'\n"
    ...
    private void look() {
    	...
    	if (!Game.player.getPrisonerComplete()) {
    		System.out.println("A woman at the far end of the square stands on the gallows, her head cast down. A small crowd has gathered.");
    	}
    	if (Game.player.getPrisonerActive()) {
    		System.out.println("She looks up and spots you. Her eyes are begging for mercy. You must stop this execution.");
    	}
    	if (Game.player.getPrisonerComplete()) {
    		System.out.println("The crowd has abandoned the now-deserted gallows, mumbling their disappointment all the way.");
    	}
    ...
    private void prisoner() {
    	if (Game.player.getPrisonerComplete()) {
    		System.out.println("The prisoner has disappeared as well as the surrounding crowd. Only two angry-looking deputies remain.\n");
    	} else if (!Game.player.getPrisonerActive()) {
